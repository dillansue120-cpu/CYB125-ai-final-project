...
def get_system_identity(snapshot):
    info = {}
    try:
        # Define the registry hive and path once so we don't repeat it
        # (you could also hardcode it in each call, but this is cleaner)
        hive = winreg.HKEY_LOCAL_MACHINE
        subkey = r"SOFTWARE\Microsoft\Windows NT\CurrentVersion"

        # Read ProductName: the friendly OS name like "Windows 11 Enterprise"
        # This is stored as a plain string in the registry, so we use it directly
        info["os_name"] = get_registry_value(hive, subkey, "ProductName")

        # Read CurrentBuild: the numeric build number like "22631"
        # Also a plain string, used directly
        info["os_build"] = get_registry_value(hive, subkey, "CurrentBuild")

        # Read EditionID: the Windows edition like "Enterprise" or "Pro"
        # Plain string, used directly
        info["os_edition"] = get_registry_value(hive, subkey, "EditionID")

        # Read RegisteredOwner: the name entered during Windows setup
        # Plain string, used directly
        info["registered_owner"] = get_registry_value(hive, subkey, "RegisteredOwner")

        # Read InstallDate: stored as a Unix timestamp integer (seconds since 1970-01-01)
        # This requires conversion to a human-readable ISO 8601 UTC string
        install_epoch = get_registry_value(hive, subkey, "InstallDate")
        if install_epoch is not None:
            # Convert the Unix timestamp to a datetime object, then format as ISO 8601 with Z suffix
            # (the "Z" suffix explicitly marks this as UTC time)
            # Alternative: you could use .strftime() instead of .isoformat() for different formatting
            info["install_date_utc"] = datetime.datetime.utcfromtimestamp(install_epoch).isoformat() + "Z"
        else:
            # If InstallDate is missing (unlikely but possible), store None instead of crashing
